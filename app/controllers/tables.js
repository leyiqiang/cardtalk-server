...
router.post('/tableOne/newSTO/:tableID', async function(req, res) {
...
router.post('/tableOne/add/:stoID', async function(req, res) {
...
router.post('/tableOne/edit/:stoSingleID', async function(req, res) {
  ...
  try {
    await changeSTOSingleData({ stoSingleID, isSuccess: reqBody.isSuccess, promptLevel: reqBody.promptLevel})
  } catch(err) {
    return res.status(500).send({message: err})
  }
})
